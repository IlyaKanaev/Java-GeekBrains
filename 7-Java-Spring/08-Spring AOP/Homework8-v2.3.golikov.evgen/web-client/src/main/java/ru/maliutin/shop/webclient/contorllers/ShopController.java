...
public class ShopController {
    ...
    public String homePage(Principal principal,
        ...
        model.addAttribute("products", shopService.getAll(principal));
        if (confirm != null){
    ...
    public String buyProduct(Principal principal,
        ...
        Product product = shopService.getAll(principal)
                .stream()
                .filter(prod -> prod.getId().equals(id))
                .findFirst()
                .orElse(null);
        BigDecimal sum = product.getPrice().multiply(new BigDecimal(amount));
        shopService.buyProduct(product.getId(), amount, sum, 1L, principal); // TODO Временно добавлен номер счета клиента
    ...
    public String errorPage(Principal principal, HttpClientErrorException e, Model model){
